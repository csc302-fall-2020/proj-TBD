...
class TestDBConnection(unittest.TestCase):
    APP = Flask(__name__)
    APP.config['MONGO_URI'] = 'mongodb+srv://{username}:{password}@{host}/{db}?retryWrites=true&w=majority'.format(
...
class TestDBQueries(unittest.TestCase):
    ...
    def test_get_form(self):
        json, response_code = app.get_form(0)

        json = json.json

        self.assertEqual(json['FormName'], 'Test1')
